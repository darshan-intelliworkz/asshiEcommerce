            ...
            @foreach(Helper::messageList() as $message)
                <a class="dropdown-item d-flex align-items-center" href="{{route('message.show',$message->id)}}">
                    <div class="dropdown-list-image mr-3">
                        @if($message->photo)
                        <img class="rounded-circle" src="{{asset('public/'.$message->photo)}}" alt="profile">
                        @else 
                        <img class="rounded-circle" src="{{asset('public/backend/img/avatar.png')}}" alt="default img">
                        @endif
  ...
  $(document).ready(function() {
    ...
    Echo.channel('message')
      .listen('MessageSent', (e) => {
        const assetBaseUrl = "{{ asset('public/') }}";
      ...
      <a class="dropdown-item d-flex align-items-center message-item" href="${e.message.url}">
        <div class="dropdown-list-image mr-3">
          <img class="rounded-circle" src="${assetBaseUrl}${e.message.photo}" alt="${assetBaseUrl}${e.message.name}">
        </div>
        <div class="font-weight-bold">
