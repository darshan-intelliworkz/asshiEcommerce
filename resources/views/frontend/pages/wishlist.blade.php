								...
								@foreach(Helper::getAllProductFromWishlist() as $key=>$wishlist)
									<tr>
										...
										<td class="product-des" data-title="Description">
											<p class="product-name"><a href="{{route('product-detail',$wishlist->product['slug'])}}">{{$wishlist->product['product_code']}}@if(isset($wishlist->color_id) && $wishlist->color_id != NULL) ({{optional($wishlist->color)->color_name}}) @endif</a></p>
											<p class="product-des">{!!($wishlist['summary']) !!}</p>
										</td>
										<td class="total-amount" data-title="Total"><span>₹{{$wishlist['amount']}}</span></td>
										<td>@if($wishlist->product['stock'] > 0)
